...
uint8_t LoRa_receive(LoRa* _LoRa, uint8_t* buffer, uint8_t* length, uint8_t max_length) {
    ...
    if (RxDoneFlag) {
        ...
        uint8_t read = LoRa_read(_LoRa, RegFiFoRxCurrentAddr);
        LoRa_write(_LoRa, RegFiFoAddPtr, read);
        uint8_t min = max_length >= number_of_bytes ? number_of_bytes : max_length;
        for (int i = 0; i < min; i++) {
            buffer[i] = LoRa_read(_LoRa, RegFiFo);
        }
        LoRa_write(_LoRa, RegIrqFlags, 0xFF); // Xóa cờ ngắt
        LoRa_gotoMode(_LoRa, RXCONTIN_MODE); // Tiếp tục nhận
        *length = min; // Lưu độ dài dữ liệu nhận được
        return min;
    }
    return 0; // Không có dữ liệu mới
}
