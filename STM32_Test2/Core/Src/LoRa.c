...
/* ----------------------------------------------------------------------------- *\
		...
		returns     : A LoRa object whith these default values:
										  ...
										  |    spreading factor = 7				       |
											...
											----------------------------------------
\* ----------------------------------------------------------------------------- */
volatile uint8_t TxDoneFlag = 0; // Cờ báo hiệu truyền hoàn tất
volatile uint8_t RxDoneFlag = 0; // Cờ báo hiệu nhận hoàn tất
...
\* ----------------------------------------------------------------------------- */
// Trả về IrqFlags (0x08 = TxDone) khi thành công, 0x00 nếu timeout
void LoRa_transmit(LoRa* _LoRa, uint8_t* data, uint8_t length)
{
    ...
    // Vào TX mode → khi phát xong sẽ có ngắt DIO0
...
\* ----------------------------------------------------------------------------- */
// Hàm LoRa_receive (sử dụng RxDoneFlag và nhận buffer từ main.c)
uint8_t LoRa_receive(LoRa* _LoRa, uint8_t* buffer, uint8_t* length, uint8_t max_length)
{
    if (_LoRa->current_mode != RXCONTIN_MODE) {
        LoRa_gotoMode(_LoRa, RXCONTIN_MODE);
        HAL_Delay(10);
    }
    ...
    /* Kiểm tra RxDone (bit 6 = 0x40) */
    if ((irqFlags & 0x40) == 0) {
        return 0; // Không có gói mới
    }

    uint8_t number_of_bytes = LoRa_read(_LoRa, RegRxNbBytes);
    uint8_t fifoAddr = LoRa_read(_LoRa, RegFiFoRxCurrentAddr);

    uint8_t to_read = (number_of_bytes <= max_length) ? number_of_bytes : max_length;

    /* Set FIFO pointer để đọc đúng offset */
    LoRa_write(_LoRa, RegFiFoAddPtr, fifoAddr);

    for (uint8_t i = 0; i < to_read; i++) {
        buffer[i] = LoRa_read(_LoRa, RegFiFo);
    }

    *length = to_read;

    /* Xóa IRQ trên module bằng cách ghi lại giá trị đã đọc */
...
/*
 * Đọc gói tin từ bộ nhớ FIFO vào buffer
 * Tự động trỏ về địa chỉ gói tin hiện tại và đọc vòng lặp
 */
void LoRa_Read_Buffer(LoRa* _LoRa, uint8_t* buffer, uint8_t length) {
    // 1. Lấy địa chỉ bắt đầu của gói tin vừa nhận
    uint8_t currentAddr = LoRa_read(_LoRa, RegFiFoRxCurrentAddr);

    // 2. Đặt con trỏ FIFO về địa chỉ đó
    LoRa_write(_LoRa, RegFiFoAddPtr, currentAddr);

    // 3. Đọc lần lượt từng byte
